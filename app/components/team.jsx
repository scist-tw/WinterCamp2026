    ...
    fetch("/data/team.json")
      ...
      .then((data) => {
        ...
        const grouped = allMembers.reduce(
          (acc, member) => {
            const cats = Array.isArray(member.category)
              ? member.category
              : [member.category];
            const roles = toArray(member.role);
  ...
  return (
    <section id="team" className="py-20 lg:py-32 px-6 lg:px-12">
      <div className="max-w-7xl mx-auto">
        ...
        {(groupedMembers["總召"]?.length > 0 ||
          groupedMembers["副召"]?.length > 0) && (
                    ...
                    <div className="bg-card text-card-foreground gap-6 border shadow-sm neon-card rounded-2xl p-6 hover:scale-[1.02] transition-transform h-full w-full relative flex flex-col">
                      <div className="flex flex-col items-center text-center flex-1">
                          ...
                          {groupedMembers["副召"][0].email ? (
                            groupedMembers["副召"][0].link?.trim() ? (
                              <button
                                ...
                                title={groupedMembers["副召"][0].name}
                        ...
                        <div className="inline-block px-3 py-1 bg-[oklch(0.75_0.15_85)] text-black text-xs font-bold rounded-full mb-3">
                          {resolveLabel(groupedMembers["副召"][0], "總召組")}
                        </div>
                        <h4 className="text-lg font-bold mb-2 line-clamp-2">
                          {groupedMembers["副召"][0].name}
                        </h4>
                        <p className="text-foreground/60 text-sm line-clamp-3">
                          {groupedMembers["副召"][0].bio ||
                            groupedMembers["副召"][0].role}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {groupedMembers["總召"]?.[0] && (
                  <div className="flex justify-center sm:col-span-1">
                    <div className="bg-card text-card-foreground gap-6 border shadow-sm neon-card rounded-2xl p-8 hover:scale-[1.04] transition-transform h-full w-full relative flex flex-col ring-2 ring-[oklch(0.75_0.15_85)]/50 sm:scale-[1.08]">
                      {resolveLabel(groupedMembers["總召"][0], "總召組") ===
                        "組長" && (
                      ...
                      <div className="flex flex-col items-center text-center flex-1">
                        <div
                          className={`relative w-38 h-36 mb-4 rounded-full overflow-hidden flex-shrink-0 transition-all duration-300 ${
                            groupedMembers["總召"][0].email &&
                            groupedMembers["總召"][0].link?.trim()
                              ? "border-4 border-[oklch(0.75_0.15_85)]/50 cursor-pointer hover:border-primary hover:scale-110 hover:shadow-lg hover:shadow-primary/50"
                          ...
                          {groupedMembers["總召"][0].email ? (
                            groupedMembers["總召"][0].link?.trim() ? (
                              <button
                                ...
                                title={groupedMembers["總召"][0].name}
                                className="w-full h-full relative group"
                              >
                                <Image
                                  src={getGravatarUrl(
                        ...
                        <p className="text-foreground/60 text-sm line-clamp-3">
                          {groupedMembers["總召"][0].bio ||
                            groupedMembers["總召"][0].role}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-center">
                    <div className="bg-card text-card-foreground gap-6 border shadow-sm neon-card rounded-2xl p-6 hover:scale-[1.02] transition-transform h-full w-full relative flex flex-col">
                      <div className="flex flex-col items-center text-center flex-1">
                          ...
                          {groupedMembers["副召"][1].email ? (
                            groupedMembers["副召"][1].link?.trim() ? (
                              <button
                                ...
                                title={groupedMembers["副召"][1].name}
                                className="w-full h-full relative group"
                              >
                                <Image
                                  src={getGravatarUrl(
                        ...
                        <div className="inline-block px-3 py-1 bg-[oklch(0.75_0.15_85)] text-black text-xs font-bold rounded-full mb-3">
                          {resolveLabel(groupedMembers["副召"][1], "總召組")}
                        </div>
                        <AutoFitText
                          as="h4"
                          className="font-bold mb-2"
                        ...
                        <p className="text-foreground/60 text-sm line-clamp-3">
                          {groupedMembers["副召"][1].bio ||
                            groupedMembers["副召"][1].role}
                        </p>
                      </div>
                    </div>
                  </div>
