    ...
    <div className="overflow-x-auto">
      <div className="min-w-[880px]">
        <div className="relative">
          <div
            className="relative grid rounded-2xl bg-card/95 border border-border/60 shadow-lg overflow-hidden"
            style={{
              gridTemplateColumns: `88px repeat(${schedule.days.length}, minmax(0, 1fr))`,
            }}
          >
            <div className="bg-muted/70 border-b border-r border-border/60 flex items-center justify-center text-xs font-semibold tracking-[0.3em] text-foreground/60">
              TIME
            </div>
            {schedule.days.map((day) => {
              ...
              return (
                <div
                  key={day.day}
                  className="bg-linear-to-b from-muted/80 via-muted/60 to-muted/30 border-b border-r border-border/60 text-center py-3"
                >
                  {meta.number ? (
                    <>
            ...
            <div className="flex flex-col bg-card/80 border-r border-border/60">
              {hourSlots.map((slot) => (
                <div
                  key={slot}
                  className="text-xs font-semibold text-foreground/60 text-right pr-3 border-b border-border/40 flex items-center justify-end"
